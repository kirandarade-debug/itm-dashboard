...
def parse_itm_content(content_str):
    ...
    return tickers_data, puts_data, calls_data


def get_all_expiry_dates(puts_data):
    """Extract all unique expiry dates from puts data"""
    expiry_dates = set()
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            ...
            dbc.ButtonGroup([
                dbc.Button("All", id="select-all", color="primary", size="sm"),
...
def update_dashboard(file_contents, select_all_expiry, clear_all_expiry, select_all, clear_all,
    ...
    tickers_data, puts_data, calls_data = parse_itm_content(data_str)

    all_expiry_dates = get_all_expiry_dates(puts_data)
    ...
    ticker_label_options = [
        {
            'label': f"{tk} ({filtered_tickers_data[tk]['num_puts']} | {format_currency(filtered_tickers_data[tk]['total_premium'])})",
            'value': tk}
    ...
    if triggered_id == "select-all":
        ticker_values = [opt['value'] for opt in ticker_label_options]
    elif triggered_id == "clear-all":
        ticker_values = []

    ticker_values = [tk for tk in ticker_values if tk in filtered_tickers_data.keys()]

    put_children = []
    for tk in ticker_values:
        if tk in filtered_puts_data:
            current_price = filtered_tickers_data[tk]['current_price']
            total_prem = format_currency(filtered_tickers_data[tk]['total_premium'])
            put_header = html.Div([
                html.H5(f"{tk} - Put Options Breakdown"),
                html.P([
                    html.Strong("Current Price: "),
                    f"${current_price:,.2f} | ",
                    html.Strong("Total Puts: "),
                    f"{len(filtered_puts_data[tk])} | ",
            ...
            for put in filtered_puts_data[tk]:
                put_children.append(
                    html.Div([
    ...
    for tk in ticker_values:
        if tk in calls_data:
            current_price = tickers_data[tk]['current_price']
            call_text = calls_data[tk]
            call_children.append(
                html.Div([
                    html.H5(f"{tk} - Call Activity Analysis"),
                    html.P([
                        html.Strong("Current Price: "),
                        f"${current_price:,.2f}"
                    ]),
    ...
    return status_msg, expiry_label_options, expiry_values, ticker_label_options, ticker_values, put_children, call_children
